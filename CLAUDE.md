...
- **Two different rating estimators: played-side vs upcoming-fixture.** `02_player_ratings_to_team.R` aggregates player ratings to team level via *different* sources depending on the row. PLAYED matches use the seasonal `ratings` table (season xRAPM → `panna`, league-centered seasonal PSR — the "displayed" estimators). UPCOMING fixtures (incl. WC2026) use a date-specific path (~line 560): `panna = offense_spm − defense_spm` (live **SPM**, not xRAPM) and `compute_player_psr(live_skills, center=TRUE)`. These are NOT the same numbers as the season ratings the blog shows per player. Fine for the XGBoost match model (it consumes home−away diffs, so any per-batch shift cancels), but do NOT reuse the match-dataset `home_sum_*` features as a *display* team rating — they won't reconcile with the squad table. The WC2026 team-strength export aggregates the displayed squad ratings instead (`12_export_wc2026_blog.R` §5, `Σ (expected_minutes/90)·player_metric`). See METRICS.md §14.
