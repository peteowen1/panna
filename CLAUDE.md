...
- **CONVENTION: positive = good, everywhere** (Pete, 2026-09-03). **Landed 2026-09-11** (merged `dev`→`main` as part of panna PR #235) — RAPM/xRAPM `defense` and `team_season_strength.parquet`'s `def_rating` now flip to positive=good at extraction (`extract_rapm_ratings()`/`extract_xrapm_ratings()`), not at each of the 5 former export sites. EPV/PSV/WPA were already positive=good. Migration plan: `pannaverse/docs/plans/SIGN-CONVENTION-POSITIVE-IS-GOOD.md`. `panna_ratings.parquet` shows `defense` as "defensive value added" (xG suppression per 90).
  ...
  - `career_panna_asof.parquet` (the predictions-pipeline's point-in-time model-feature file) is still on the pre-flip vintage as of 2026-09-11 — regenerating it needs its own run of `data-raw/estimated-skills/09b_career_panna_asof.R` (smoke-tested, ~1.5hrs for the full ~150-monthly-snapshot regen), not yet executed at full scale.
