...
- **Cross-league PSR offsets are TRANSFER-based (not Elo/opponent), because PSR is intrinsically league-flat.** PSR is a dot-product of box-score skill *rates* × glmnet betas; those rates barely vary by league (a mid completes ~80% of passes in the A-League and the EPL alike), so a strong player in a weak league posts an inflated PSR and tops the sortable column (the 2026-06 "unknown WC players #1 in PSR" report). The fix is `compute_psr_league_offsets()` (psr.R): a per-league additive offset estimated from **adjacent-season player transfers** (`Δpsr ≈ ease_to − ease_from` over a 1-yr window kills the career-stage confound), Big-5 anchored, solved by weighted ridge; `apply_psr_league_offsets()` adds it (and splits across osr/dsr to keep `osr+dsr=psr`). Applied **display-only** in step `06` (seasonal, saves `cache-skills/psr_league_offsets.parquet`+csv) and step `08b` (weekly, *reads* that parquet so both use identical offsets — 06 must run first; 08b attaches each player's as-of-date primary league via a 365-day trailing window). **Why not Elo/opponent-adjustment (the EPR/Panna mechanism):** the prediction Elo has **0% coverage** for exactly the problem leagues (A_League, Brazilian_Serie_A, Belgian_First_Division, CAF_CL) — match-based calibration can't reach leagues with no shared fixtures, so opponent adjustment falls back to `opp_factor=1` and does nothing. **Transfers are the only bridge** to these isolated leagues. EPR/Panna avoid the problem because their signals (EPV, on-pitch xG-margin) are naturally league-sensitive and they control opponents *in-regression*; PSR has neither. Offsets are deliberately **modest** (A_League ≈ −0.06 on the PSR scale, two independent estimators agree) — that's genuinely all the data supports; it moves the weak-league names out of the top-20 but can't bury them harder without a curated/external league-strength input. The RAPM `psvf90` target and match-prediction `home_sum_psr` features are UNTOUCHED (offsets live only in the two display exports). The old in-league-MEAN `league_baseline` idea was a dead end: a weak league's cross-sectional PSR mean ≈ 0 because weak-players × easy-league cancel, so centering on it does nothing — the bridge holds the player fixed and isolates league ease.
