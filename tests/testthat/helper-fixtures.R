...
#' Load a real-match fixture (West Ham 3-0 Leeds, EPL 2026-05-24)
#'
#' Generated by data-raw/make_spadl_fixture.R ("events", "spadl") and
#' data-raw/make_epv_fixture.R ("spadl_labeled", "epv_input"):
#'   events        raw Opta match events (input to convert_opta_to_spadl)
#'   spadl         converted SPADL actions, all columns
#'   spadl_labeled SPADL + possession chains + next-goal/next-xG labels
#'                 (the `labels` arg of fit_epv_model)
#'   epv_input     create_epv_features_simple() output
#'                 (the `features` arg of fit_epv_model)
load_real_match_fixture <- function(what = c("spadl", "events",
                                             "spadl_labeled", "epv_input")) {
  what <- match.arg(what)
  testthat::skip_if_not_installed("arrow")
  path <- testthat::test_path("fixtures", switch(what,
    spadl         = "spadl_match_epl.parquet",
    events        = "opta_match_events_epl.parquet",
    spadl_labeled = "spadl_labeled_epl.parquet",
    epv_input     = "epv_input_epl.parquet"
  ))
  if (!file.exists(path)) {
    testthat::skip(paste0("real-match fixture not generated (data-raw/make_",
                          if (what %in% c("spadl", "events")) "spadl" else "epv",
                          "_fixture.R)"))
  }
