...
# 1. Loading base RAPM, SPM, and xRAPM results
# 2. Creating unified player ratings table
...
panna_ratings <- xrapm_results$ratings %>%
  select(
    ...
    player_name,
    total_minutes,
    # xRAPM is the final Panna rating
    panna = xrapm,
    offense = offense,
    defense = defense,
    # Deviation from SPM prior (data-driven adjustment)
    off_deviation,
    def_deviation,
    # SPM priors
    off_prior,
...
# Top defensive players (lowest defense = best, allows least xG)
...
# Players most different from SPM prior
cat("\nPlayers who outperformed SPM (positive deviation):\n")
print(
  panna_ratings %>%
    mutate(total_deviation = off_deviation - def_deviation) %>%
    arrange(desc(total_deviation)) %>%
    head(15) %>%
    select(player_name, panna, spm_overall, total_deviation, total_minutes) %>%
    mutate(across(where(is.numeric) & !matches("minutes"), ~round(.x, 3)))
)

cat("\nPlayers who underperformed SPM (negative deviation):\n")
print(
  panna_ratings %>%
    mutate(total_deviation = off_deviation - def_deviation) %>%
