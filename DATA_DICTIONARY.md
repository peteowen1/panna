...
> **Archived 2026-04** (commit 43ea7fd): the FBref pipeline is retired —
> Opta is the only active data source. Steps listed for historical
> reference; the scripts live under `archive/` and are not maintained.
...
> **The FBref pipeline is retired** (archived 2026-04, commit 43ea7fd).
> These schema tables are kept for reading older pannadata releases that
> still use them — the pipeline itself no longer runs.
...
Two-path system: EPV (action-level) + PSV (results-priced box-score) +
`spm_value` (context-priced box-score) → `piero_value`.
